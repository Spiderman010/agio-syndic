...
--
-- Verwachte uitkomst: "35 geslaagd, 0 gefaald".
--
-- Bijgewerkt door m23: M11 is gesplitst in M11a/M11b met exacte foutcodes, M33 is vervangen
-- door een zelfstandige controle op de bladtabellen, en afsluiten loopt via close_fiscal_year().
--
...
BEGIN
  ...
  PERFORM public.close_fiscal_year(vfy25);
  ...
  -- heeft twee aparte takken en vuurt eerder in de cascade dan de betalingsguard. Beide takken
  -- worden hieronder LOS bewezen met een exacte foutcode; een OR-assertie over twee codes zou
  -- verhullen welke guard er werkelijk vuurt. m22's betalingsguard zit daar als tweede slot
  -- achter en wordt zelfstandig bewezen in M07 t/m M10 -- op deze route kan hij per definitie
  -- nooit als eerste vuren. Dat m22 GEEN escape op owners kreeg is precies hierom: zonder eigen
  -- ouderguard zou zo'n escape de betalingsguard gratis omzeilbaar maken.
  PERFORM set_config('request.jwt.claims',PA,true);
  BEGIN DELETE FROM public.owners WHERE id=vo; ok := false;
  EXCEPTION WHEN others THEN ok := (SQLERRM LIKE 'ALLOC_OWNER_HAS_HISTORY%'); msg := left(SQLERRM,80); END;
  IF ok THEN pass:=pass+1; ELSE fail:=fail+1; END IF;
  rep := rep || E'\n' || CASE WHEN ok THEN 'PASS' ELSE 'FAIL' END
             || '  M11a eigenaar MET vorderingen -> ALLOC_OWNER_HAS_HISTORY (vorderingentak m18)';

  -- Eigenaar zonder lot en dus zonder vorderingen, met uitsluitend een betaling: hier vuurt
  -- aantoonbaar de betalingstak van diezelfde ouderguard, met een EIGEN exacte foutcode.
  DECLARE vo2 uuid; vbP uuid; vfyP uuid;
  BEGIN
    INSERT INTO public.buildings(organization_id,name,total_tantiemes) VALUES (vorg,'M11B',100) RETURNING id INTO vbP;
    INSERT INTO public.fiscal_years(organization_id,building_id,year,start_date,end_date,status)
    VALUES (vorg,vbP,2028,'2028-01-01','2028-12-31','open') RETURNING id INTO vfyP;
    INSERT INTO public.owners(organization_id,full_name) VALUES (vorg,'Zonder lot') RETURNING id INTO vo2;
    INSERT INTO public.payments(organization_id,building_id,owner_id,amount,method,value_date)
    VALUES (vorg,vbP,vo2,250.00,'virement','2028-04-01');
    SELECT count(*) INTO n FROM public.charge_allocations WHERE owner_id=vo2;
    BEGIN DELETE FROM public.owners WHERE id=vo2; ok := false;
    EXCEPTION WHEN others THEN ok := (SQLERRM LIKE 'ALLOC_OWNER_HAS_PAYMENTS%'); msg := left(SQLERRM,80); END;
  END;
  ok := ok AND (n = 0);
  IF ok THEN pass:=pass+1; ELSE fail:=fail+1; END IF;
  rep := rep || E'\n' || CASE WHEN ok THEN 'PASS' ELSE 'FAIL' END
             || '  M11b eigenaar met UITSLUITEND een betaling (' || n || ' vorderingen) -> ALLOC_OWNER_HAS_PAYMENTS';
  ...
  PERFORM public.close_fiscal_year(vfy3);
  ...
  -- Zelfstandige controle op de BLADtabellen. M32 telt de koptabellen; hier wordt met een
  -- eigen query geverifieerd dat de cascade ook opruimt wat daaronder hangt en nergens
  -- anders in deze suite wordt geteld.
  SELECT (select count(*) from public.charge_allocations  where organization_id=vorg3)
       + (select count(*) from public.payment_allocations where organization_id=vorg3)
       + (select count(*) from public.journal_lines       where organization_id=vorg3)
       + (select count(*) from public.fund_movements      where organization_id=vorg3)
       + (select count(*) from public.documents           where organization_id=vorg3)
       + (select count(*) from public.expenses            where organization_id=vorg3) INTO n;
  ok := (n = 0);
  IF ok THEN pass:=pass+1; ELSE fail:=fail+1; END IF;
  rep := rep || E'\n' || CASE WHEN ok THEN 'PASS' ELSE 'FAIL' END
             || '  M33  offboarding laat ook de bladtabellen leeg (' || n || ' restanten)';
