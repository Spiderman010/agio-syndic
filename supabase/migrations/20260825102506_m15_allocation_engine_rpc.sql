-- m15 — Flexible allocation engine, deel 4: de atomische RPC
--
-- Vervangt de trigger-gedreven creatie. Alles gebeurt in EEN transactie en in
-- EEN functie: regelvalidatie, scope- en eigenaarsresolutie, snapshot,
-- centallocatie, somcontrole en journaalpost.
--
-- Waarom dit geen triggers meer zijn:
--   * De oude trig_01 (allocatie) en trig_02 (journaal) vuurden op AFTER INSERT
--     van charge_calls. Handmatige verdeelregels kunnen op dat moment per
--     definitie nog niet bestaan, dus `manual` was in die opzet onuitvoerbaar.
--   * De snapshotkop was via PostgREST door de client te vullen; de kop kon dus
--     iets anders zeggen dan de regel die daadwerkelijk is gebruikt.
-- Beide problemen verdwijnen zodra er precies EEN schrijfpad is dat de volgorde
-- bepaalt en alle snapshotvelden zelf berekent.
...
BEGIN
  ...
  IF v_rule.method = 'percentage' AND v_den <> 100000000 THEN
    RAISE EXCEPTION 'ALLOC_PCT_SUM: de percentages tellen op tot %%%, niet tot 100%%',
  ...
  -- ---- 10. snapshotkop ----------------------------------------------------
  SELECT (p_total_amount * 0)::bigint INTO v_rtot;   -- placeholder, gevuld na verdeling
  SELECT coalesce(bl.code, NULL) INTO v_block_code
    FROM public.blocks bl WHERE bl.id = v_rule.scope_block_id;
...
-- Enig schrijfpad: rechtstreeks invoegen of wijzigen van een lastenoproep is
-- vanaf nu geblokkeerd. Daarmee kan de snapshotkop niet meer door de client
-- worden gevuld en kan hij dus nooit iets anders zeggen dan de gebruikte regel.
...
COMMENT ON FUNCTION public.create_charge_call(uuid, public.charge_call_type, numeric, date, date, text, text, text, uuid, jsonb) IS
  'Enig schrijfpad voor een lastenoproep. Doet validatie, resolutie, snapshot, centverdeling, somcontrole en journaalpost in een transactie. Faalt hard bij elke onvolledigheid; kent geen stille terugval.';
